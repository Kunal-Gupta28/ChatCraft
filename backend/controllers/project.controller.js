...
const {createProject,getAllProjectByUserId, addUserToProject, removeUserFromProject, updateFileTree} = require("../services/project.service")
...
module.exports.addUserToProject = async (req,res)=>{
    ...
    try {
        ...
        const isLoggedInUser = await userModel.findOne({email:req.user.email})
        const updatedProject = await addUserToProject({projectId,users,userId:isLoggedInUser._id})
...
module.exports.updateFileTree = async(req,res) => {
    ...
    try {
        const updatedProject = await updateFileTree({projectId, updatedfile, newCode});
